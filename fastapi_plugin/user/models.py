...
from fastapi_plugin.crud.sqlmodel import Relationship, select, SQLModel, Field
...
class UsernameMixin(SQLModel):
    username: str = Field(title="Username", max_length=32, unique=True, index=True, nullable=False)
...
class PasswordMixin(SQLModel):
    password: PasswordStr = Field(title="Password", max_length=128, nullable=False)
...
class EmailMixin(SQLModel):
    ...
    email: EmailStr = Field(None, title="Email", index=True, nullable=True)
...
class BaseUser(PkMixin, UsernameMixin, PasswordMixin, EmailMixin, CreateTimeMixin):
    ...
    nickname: Optional[str] = Field(None, title="Nickname", max_length=40, nullable=True)
...
class BaseRBAC(PkMixin):
    ...
    key: str = Field(title="Identify", max_length=40, unique=True, index=True, nullable=False)
    name: str = Field(default="", title="Name", max_length=40)
    desc: str = Field(default="", title="Description", max_length=400)
